...
##### Вариант 0: Synology DSM Reverse Proxy

Если вы используете Synology NAS, см. подробную инструкцию в файле `SYNOLOGY_REVERSE_PROXY.md`.

Кратко:
1. **Панель управления** → **Логин-портал** → **Обратный прокси-сервер**
2. Создайте правило:
   - **Источник:** HTTPS, ваш домен, порт 443
   - **Назначение:** HTTP, внутренний IP сервера с Django, порт 8000
3. Настройте SSL сертификат для домена
4. Включите передачу заголовков `X-Forwarded-Proto` и `X-Forwarded-Host`
...
2. **Отредактируйте конфигурацию:**
   ...
   - **Для внешнего сервера (Вариант 2):** `192.168.1.100` на внутренний IP адрес сервера с Django
   - Пути к SSL сертификатам (если используете не Let's Encrypt)
   
   **Важно для внешнего сервера:**
   - Django Admin должен слушать на `0.0.0.0:8000` (не только `127.0.0.1`) - это уже настроено в systemd сервисе
   - Внутренний сервер должен быть доступен из локальной сети
   - Firewall на внутреннем сервере должен разрешать подключения с внешнего сервера
